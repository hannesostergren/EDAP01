...
API_KEY = 'nyckel'
STIL_ID = ["ha6882os-s"] # TODO: fill this list with your stil-id's
...
def max_function(env, alpha, beta, depth, t1): 
   if depth == 0 or env.is_win_state() or time.time()-t1 > 4.7:
      return eval(env)
   v = -np.inf
   for successiveMove in list(env.available_moves()):
      successiveState = copy.deepcopy(env)
      successiveState.step(successiveMove)
      successiveState.change_player()
      v = max(v, min_function(successiveState, alpha, beta, depth-1, t1))
      alpha = max(alpha, v)
      if v >= beta: 
         return v 
   return v
...
def min_function(env, alpha, beta, depth, t1): 
   ...
   for successiveMove in list(env.available_moves()):
      ...
      successiveState.step(successiveMove)
      successiveState.change_player()
      v = min(v, max_function(successiveState, alpha, beta, depth-1, t1))
      beta = min(beta, v)
      if v <= alpha: 
         return v 
   return v

def extractBlocks(env):
   score = 0 
   ...
   for i in range(env.board_shape[0]):
      for j in range(env.board_shape[1] - 3):
         ...
         score += eval4(prevSquares,1)

   reversed_board = np.transpose(env.board)
...
def student_move(env, moveNum):
   ...
   for move in avMovs:      
      ...
      if t2-t1 > 4.7:
         break
      successiveState = copy.deepcopy(env)
      successiveState.step(move)
      successiveState.change_player()
      score = min_function(successiveState, initialAlpha, initialBeta, maxDepth, t1)
      if move == avMovs[0]:
         maxScore = score
      if score == maxScore:
